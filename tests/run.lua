...
-- focus the outline: enter, dd to remove, l to jump back out
miniharp.focus_pin()
assert(ui.is_pin_open(), 'focus should open the outline')
assert(vim.api.nvim_get_current_win() == state.pin_win, 'focus should enter the outline')
assert(vim.api.nvim_win_get_cursor(state.pin_win)[1] == 2, 'cursor should start on the current file row')
miniharp.focus_pin()
assert(vim.api.nvim_get_current_win() ~= state.pin_win, 'focusing again should leave the outline')
assert(ui.is_pin_open(), 'the outline stays open after leaving')
miniharp.focus_pin()
vim.api.nvim_win_set_cursor(state.pin_win, { 1, 0 })
feed('dd')
assert(#state.marks == 1, 'dd in the outline should remove a mark')
feed('l')
assert(vim.api.nvim_get_current_win() ~= state.pin_win, 'l should jump and leave the outline')
assert(ui.is_pin_open(), 'the outline stays open after jumping')
assert(vim.fn.expand('%:t') == 'core.lua', 'l should open the mark under the cursor')

-- reorder with <C-j>/<C-k> inside the outline
vim.cmd('edit lua/miniharp/init.lua')
miniharp.toggle_file()
...
feed('q')
assert(vim.api.nvim_get_current_win() ~= state.pin_win, 'q should leave the outline')
miniharp.toggle_pin()
assert(not ui.is_pin_open(), 'toggle should close the outline')
vim.cmd('edit lua/miniharp/init.lua')
miniharp.toggle_file()
vim.cmd('edit lua/miniharp/core.lua')
